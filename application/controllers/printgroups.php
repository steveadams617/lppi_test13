...
class Printgroups extends MY_Controller 
{
	...
	private $default_segments = array('sort_by'=>'id', 'sort_order'=>'asc', 'search_text'=>'', 'page'=>false );

	private $table_display_name = 'Printgroups' ;
	private $table_name = 'printgroups';
	private $frame_name = 'frame_admin.tpl';
	private $navigation_file = 'navigation_default.tpl';
	
	function __construct()
	{
		parent::__construct();

		$this->load->model( 'model_printgroups' ); 
    ...
    function index()
    {
		...
		if ($_SERVER ['REQUEST_METHOD']=='POST') {
			...
			redirect(base_url() . 'printgroups/index' . uri_segments($this->segments) );
		}

		if (!isset($this->segments['sort_by'])) {
			if ( isset($this->session->userdata['uri_segments']['printgroups'] )) {
					$this->segments = $this->session->userdata['uri_segments']['printgroups'];
				...
				$this->session->userdata['uri_segments']['printgroups'] = $this->segments;
				$this->session->sess_write();
				$this->logToFile('steve.txt', 'After saving to session after filling default values:' );
				$this->logToFile('steve.txt', print_r($this->session->userdata('printgroups_uri_segments'), TRUE) );
			}
		} else {
			$this->segments = fill_default_values($this->segments, $this->default_segments);
			$this->session->userdata['uri_segments']['printgroups'] = $this->segments;
    	...
    	$this->action = "list";
		$this->template->assign( 'data', $this->model_printgroups->lister( $this->segments, $this->pagination_per_page ) );
		
		
		$this->template->assign( 'pager', $this->pager($this->segments) );
		$this->template->assign( 'partial_path', 'printgroups' );
    ...
    function show( $id )
    {
		...
		$this->template->assign( 'data', $this->model_printgroups->get( $id ) );
    ...
    function delete( $id = FALSE )
    {
        switch ( $_SERVER ['REQUEST_METHOD'] )
        {
            case 'GET':
                ...
                $this->model_printgroups->delete( $id );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
 			case 'POST':
 				//delete from submit button on list using checked boxes
                $this->model_printgroups->delete( $this->input->post('delete_ids') );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
		}
	}
    ...
    protected function setRules()
	{
		...
		$this->load->library('form_validation');
						$this->form_validation->set_rules( 'user_id', lang('user_id'), 'required|max_length[6]|integer' );
				$this->form_validation->set_rules( 'print_timestamp', lang('print_timestamp'), 'required' );
	...
	protected function getPostData($action)
	{
				...
				$data_post['print_timestamp'] = $this->input->post( 'print_timestamp' );
	...
	{	
		...
		$this->template->assign( 'fields', $this->model_printgroups->fields( $includePrimaryIDfield ) );
	...
	protected function displayFormForEdit($id){
		if ($this->action == "edit")
		{
			$this->model_printgroups->raw_data = TRUE;
			$this->template->assign( 'data', $this->model_printgroups->get( $id ) );
	...
	protected function saveFormData($id){
		switch ($this->action) {
			case 'create':
				...
				$insert_id = $this->model_printgroups->insert( $data );
				
				redirect(base_url() . 'printgroups/show/' . $insert_id );
				break;
			case 'edit':
				$this->model_printgroups->update( $id, $this->getPostData('save') );
				redirect(base_url() . 'printgroups/show/' . $id );   
				break;
		}	
	...
	protected function commonAssignments()
	{
		...
		$this->template->assign( 'metadata', $this->model_printgroups->metadata() );
	}
	...
	protected function pager(){
        ...
        {
        	$this->total_rows = $this->model_printgroups->total_rows($this->segments['search_text']);
        	unset($this->segments['page']);
        	
            $config = array();
            $config['uri_segment'] = 8;
			$config['total_rows'] = $this->total_rows;
			$config['base_url'] = base_url() . 'printgroups/index' .  uri_segments($this->segments) .  '/page/';
