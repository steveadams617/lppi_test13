...
class Teams extends MY_Controller 
{
	...
	private $default_segments = array('sort_by'=>'team', 'sort_order'=>'asc', 'search_text'=>'', 'page'=>false );

	private $table_display_name = 'Teams' ;
    ...
    function index()
    {
		...
		if ($_SERVER ['REQUEST_METHOD']=='POST') {
			...
			redirect(base_url() . 'Lookup/teams/index' . uri_segments($this->segments) );
		}

		if (!isset($this->segments['sort_by'])) {
			if ( isset($this->session->userdata['uri_segments']['teams'] )) {
					$this->segments = $this->session->userdata['uri_segments']['teams'];
				...
				$this->session->userdata['uri_segments']['teams'] = $this->segments;
				$this->session->sess_write();
				$this->logToFile('steve.txt', 'After saving to session after filling default values:' );
				$this->logToFile('steve.txt', print_r($this->session->userdata('teams_uri_segments'), TRUE) );
			}
		} else {
			$this->segments = fill_default_values($this->segments, $this->default_segments);
			$this->session->userdata['uri_segments']['teams'] = $this->segments;
    	...
    	$this->action = "list";
		$this->template->assign( 'data', $this->model_teams->lister( $this->segments, $this->pagination_per_page ) );
		
		
		$this->template->assign( 'pager', $this->pager($this->segments) );
		$this->template->assign( 'partial_path', 'Lookup/teams' );
    ...
    function show( $id )
    {
		...
		$this->template->assign( 'data', $this->model_teams->get( $id ) );
    ...
    function delete( $id = FALSE )
    {
        switch ( $_SERVER ['REQUEST_METHOD'] )
        {
            case 'GET':
                ...
                $this->model_teams->delete( $id );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
 			case 'POST':
 				//delete from submit button on list using checked boxes
                $this->model_teams->delete( $this->input->post('delete_ids') );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
		}
	}
    ...
    protected function setRules()
	{
		...
		$this->load->library('form_validation');
						$this->form_validation->set_rules( 'team', lang('team'), 'required|max_length[5]' );
	...
	{
						$data_post['team'] = $this->input->post( 'team' );
	...
	{	
		...
		$this->template->assign( 'fields', $this->model_teams->fields( $includePrimaryIDfield ) );
	...
	protected function displayFormForEdit($id){
		if ($this->action == "edit")
		{
			$this->model_teams->raw_data = TRUE;
			$this->template->assign( 'data', $this->model_teams->get( $id ) );
	...
	protected function saveFormData($id){
		switch ($this->action) {
			case 'create':
				...
				$insert_id = $this->model_teams->insert( $data );
				
				redirect(base_url() . 'Lookup/teams/show/' . $insert_id );
				break;
			case 'edit':
				$this->model_teams->update( $id, $this->getPostData('save') );
				redirect(base_url() . 'Lookup/teams/show/' . $id );   
				break;
		}	
	...
	protected function commonAssignments()
	{
		...
		$this->template->assign( 'metadata', $this->model_teams->metadata() );
	}
	...
	protected function pager(){
        ...
        {
        	$this->total_rows = $this->model_teams->total_rows($this->segments['search_text']);
        	unset($this->segments['page']);
        	
            $config = array();
            $config['uri_segment'] = 9;
			$config['total_rows'] = $this->total_rows;
			$config['base_url'] = base_url() . 'Lookup/teams/index' .  uri_segments($this->segments) .  '/page/';
