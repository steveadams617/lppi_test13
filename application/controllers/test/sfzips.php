...
class Sfzips extends MY_Controller 
{
	...
	private $default_segments = array('sort_by'=>'sfz_zip', 'sort_order'=>'asc', 'search_text'=>'', 'page'=>false );

	private $table_display_name = 'Sfzips' ;
	private $table_name = 'sfzips';
    ...
    function index()
    {
		...
		if ($_SERVER ['REQUEST_METHOD']=='POST') {
			...
			redirect(base_url() . 'test/sfzips/index' . uri_segments($this->segments) );
		}

		if (!isset($this->segments['sort_by'])) {
			if ( isset($this->session->userdata['uri_segments']['sfzips'] )) {
					$this->segments = $this->session->userdata['uri_segments']['sfzips'];
				...
				$this->session->userdata['uri_segments']['sfzips'] = $this->segments;
				$this->session->sess_write();
				$this->logToFile('steve.txt', 'After saving to session after filling default values:' );
				$this->logToFile('steve.txt', print_r($this->session->userdata('sfzips_uri_segments'), TRUE) );
			}
		} else {
			$this->segments = fill_default_values($this->segments, $this->default_segments);
			$this->session->userdata['uri_segments']['sfzips'] = $this->segments;
    	...
    	$this->action = "list";
		$this->template->assign( 'data', $this->model_sfzips->lister( $this->segments, $this->pagination_per_page ) );
		
		
		$this->template->assign( 'pager', $this->pager($this->segments) );
		$this->template->assign( 'partial_path', 'test/sfzips' );
    ...
    function show( $id )
    {
		...
		$this->template->assign( 'data', $this->model_sfzips->get( $id ) );
    ...
    function delete( $id = FALSE )
    {
        switch ( $_SERVER ['REQUEST_METHOD'] )
        {
            case 'GET':
                ...
                $this->model_sfzips->delete( $id );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
 			case 'POST':
 				//delete from submit button on list using checked boxes
                $this->model_sfzips->delete( $this->input->post('delete_ids') );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
		}
	}
    ...
    protected function setRules()
	{
		...
		$this->load->library('form_validation');
						$this->form_validation->set_rules( 'sfz_zip', lang('sfz_zip'), 'required|max_length[5]' );
				$this->form_validation->set_rules( 'sfz_neighhoods', lang('sfz_neighhoods'), 'required|max_length[200]' );
	...
	{
						$data_post['sfz_zip'] = $this->input->post( 'sfz_zip' );
				$data_post['sfz_neighhoods'] = $this->input->post( 'sfz_neighhoods' );
	...
	{	
		...
		$this->template->assign( 'fields', $this->model_sfzips->fields( $includePrimaryIDfield ) );
	...
	protected function displayFormForEdit($id){
		if ($this->action == "edit")
		{
			$this->model_sfzips->raw_data = TRUE;
			$this->template->assign( 'data', $this->model_sfzips->get( $id ) );
	...
	protected function saveFormData($id){
		switch ($this->action) {
			case 'create':
				...
				$insert_id = $this->model_sfzips->insert( $data );
				
				redirect(base_url() . 'test/sfzips/show/' . $insert_id );
				break;
			case 'edit':
				$this->model_sfzips->update( $id, $this->getPostData('save') );
				redirect(base_url() . 'test/sfzips/show/' . $id );   
				break;
		}	
	...
	protected function commonAssignments()
	{
		...
		$this->template->assign( 'metadata', $this->model_sfzips->metadata() );
	}
	...
	protected function pager(){
        ...
        {
        	$this->total_rows = $this->model_sfzips->total_rows($this->segments['search_text']);
        	unset($this->segments['page']);
        	
            $config = array();
            $config['uri_segment'] = 9;
			$config['total_rows'] = $this->total_rows;
			$config['base_url'] = base_url() . 'test/sfzips/index' .  uri_segments($this->segments) .  '/page/';
