...
class Episodes extends MY_Controller 
{
	...
	private $default_segments = array('sort_by'=>'patients.first_name', 'sort_order'=>'asc', 'search_text'=>'', 'page'=>false );

	private $table_display_name = 'Episodes' ;
	...
	function __construct()
	{
		...
		$this->load->model( array('model_episodes', 'model_teams') ); 
    ...
    function index()
    {
		...
		if ($_SERVER ['REQUEST_METHOD']=='POST') {
			...
			redirect(base_url() . 'test/episodes/index' . uri_segments($this->segments) );
		}

		if (!isset($this->segments['sort_by'])) {
			if ( isset($this->session->userdata['uri_segments']['episodes'] )) {
					$this->segments = $this->session->userdata['uri_segments']['episodes'];
				...
				$this->session->userdata['uri_segments']['episodes'] = $this->segments;
				$this->session->sess_write();
				$this->logToFile('steve.txt', 'After saving to session after filling default values:' );
				$this->logToFile('steve.txt', print_r($this->session->userdata('episodes_uri_segments'), TRUE) );
			}
		} else {
			$this->segments = fill_default_values($this->segments, $this->default_segments);
			$this->session->userdata['uri_segments']['episodes'] = $this->segments;
    	...
    	$this->action = "list";
		$this->template->assign( 'data', $this->model_episodes->lister( $this->segments, $this->pagination_per_page ) );
		
		
		$this->template->assign( 'pager', $this->pager($this->segments) );
		$this->template->assign( 'partial_path', 'test/episodes' );
    ...
    function show( $id )
    {
    	$this->session->set_userdata('episode_id', $id);
		// Start add
		$this->session->set_userdata('current_patient_name', $this->model_episodes->patient_name($id) );
		// End add
		$this->action = "show";
		$this->template->assign( 'id', $id );
		$this->template->assign( 'data', $this->model_episodes->get( $id ) );
    ...
    function delete( $id = FALSE )
    {
        switch ( $_SERVER ['REQUEST_METHOD'] )
        {
            case 'GET':
                ...
                $this->model_episodes->delete( $id );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
 			case 'POST':
 				//delete from submit button on list using checked boxes
                $this->model_episodes->delete( $this->input->post('delete_ids') );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
		}
	}
    ...
    protected function setRules()
	{
		...
		$this->load->library('form_validation');
						$this->form_validation->set_rules( 'team_id', lang('team_id'), 'required|max_length[10]|integer' );
	...
	{
						$data_post['team_id'] = $this->input->post( 'team_id' );
				$data_post['oneliner'] = $this->input->post( 'oneliner' );
				$data_post['start_date'] = MySQLDate( $this->input->post( 'start_date' ) );
				$data_post['end_date'] = MySQLDate( $this->input->post( 'end_date' ) );
				$data_post['historyphysical'] = $this->input->post( 'historyphysical' );
	...
	{	
		...
		$this->template->assign( 'fields', $this->model_episodes->fields( $includePrimaryIDfield ) );
	...
	protected function displayFormForEdit($id){
		if ($this->action == "edit")
		{
			$this->model_episodes->raw_data = TRUE;
			$this->template->assign( 'data', $this->model_episodes->get( $id ) );
	...
	protected function saveFormData($id){
		switch ($this->action) {
			case 'create':
				...
				$insert_id = $this->model_episodes->insert( $data );
				$this->session->set_userdata('episode_id', $insert_id);

				redirect(base_url() . 'main/notes' );
				break;
			case 'edit':
				$this->model_episodes->update( $id, $this->getPostData('save') );
				redirect(base_url() . 'test/episodes/show/' . $id );   
				break;
		}	
	...
	protected function commonAssignments()
	{
		...
		$this->template->assign( 'metadata', $this->model_episodes->metadata() );
	}
	...
	protected function setRelated()
	{
        ...
        $this->template->assign( 'related_teams', $this->model_teams->related_team());
	...
	protected function pager(){
        ...
        {
        	$this->total_rows = $this->model_episodes->total_rows($this->segments['search_text']);
        	unset($this->segments['page']);
        	
            $config = array();
            $config['uri_segment'] = 9;
			$config['total_rows'] = $this->total_rows;
			$config['base_url'] = base_url() . 'test/episodes/index' .  uri_segments($this->segments) .  '/page/';
