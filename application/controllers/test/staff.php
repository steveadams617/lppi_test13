...
class Staff extends MY_Controller 
{
	...
	private $default_segments = array('sort_by'=>'st_room', 'sort_order'=>'asc', 'search_text'=>'', 'page'=>false );

	private $table_display_name = 'Staff' ;
    ...
    function index()
    {
		...
		if ($_SERVER ['REQUEST_METHOD']=='POST') {
			...
			redirect(base_url() . 'test/staff/index' . uri_segments($this->segments) );
		}

		if (!isset($this->segments['sort_by'])) {
			if ( isset($this->session->userdata['uri_segments']['staff'] )) {
					$this->segments = $this->session->userdata['uri_segments']['staff'];
				...
				$this->session->userdata['uri_segments']['staff'] = $this->segments;
				$this->session->sess_write();
				$this->logToFile('steve.txt', 'After saving to session after filling default values:' );
				$this->logToFile('steve.txt', print_r($this->session->userdata('staff_uri_segments'), TRUE) );
			}
		} else {
			$this->segments = fill_default_values($this->segments, $this->default_segments);
			$this->session->userdata['uri_segments']['staff'] = $this->segments;
    	...
    	$this->action = "list";
		$this->template->assign( 'data', $this->model_staff->lister( $this->segments, $this->pagination_per_page ) );
		
		
		$this->template->assign( 'pager', $this->pager($this->segments) );
		$this->template->assign( 'partial_path', 'test/staff' );
    ...
    function show( $id )
    {
		...
		$this->template->assign( 'data', $this->model_staff->get( $id ) );
    ...
    function delete( $id = FALSE )
    {
        switch ( $_SERVER ['REQUEST_METHOD'] )
        {
            case 'GET':
                ...
                $this->model_staff->delete( $id );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
 			case 'POST':
 				//delete from submit button on list using checked boxes
                $this->model_staff->delete( $this->input->post('delete_ids') );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
		}
	}
    ...
    protected function setRules()
	{
		...
		$this->load->library('form_validation');
						$this->form_validation->set_rules( 'Column 2', lang('Column 2'), 'required|max_length[9]|integer' );
				$this->form_validation->set_rules( 'st_room', lang('st_room'), '10' );
	...
	{
						$data_post['Column 2'] = $this->input->post( 'Column 2' );
				$data_post['st_room'] = $this->input->post( 'st_room' );
	...
	{	
		...
		$this->template->assign( 'fields', $this->model_staff->fields( $includePrimaryIDfield ) );
	...
	protected function displayFormForEdit($id){
		if ($this->action == "edit")
		{
			$this->model_staff->raw_data = TRUE;
			$this->template->assign( 'data', $this->model_staff->get( $id ) );
	...
	protected function saveFormData($id){
		switch ($this->action) {
			case 'create':
				...
				$insert_id = $this->model_staff->insert( $data );
				
				redirect(base_url() . 'test/staff/show/' . $insert_id );
				break;
			case 'edit':
				$this->model_staff->update( $id, $this->getPostData('save') );
				redirect(base_url() . 'test/staff/show/' . $id );   
				break;
		}	
	...
	protected function commonAssignments()
	{
		...
		$this->template->assign( 'metadata', $this->model_staff->metadata() );
	}
	...
	protected function pager(){
        ...
        {
        	$this->total_rows = $this->model_staff->total_rows($this->segments['search_text']);
        	unset($this->segments['page']);
        	
            $config = array();
            $config['uri_segment'] = 9;
			$config['total_rows'] = $this->total_rows;
			$config['base_url'] = base_url() . 'test/staff/index' .  uri_segments($this->segments) .  '/page/';
