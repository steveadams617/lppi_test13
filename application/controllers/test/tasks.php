...
class Tasks extends MY_Controller 
{
	...
	private $default_segments = array('sort_by'=>'task', 'sort_order'=>'asc', 'search_text'=>'', 'page'=>false );

	private $table_display_name = 'Tasks' ;
    ...
    function index()
    {
		...
		if ($_SERVER ['REQUEST_METHOD']=='POST') {
			...
			redirect(base_url() . 'test/tasks/index' . uri_segments($this->segments) );
		}

		if (!isset($this->segments['sort_by'])) {
			if ( isset($this->session->userdata['uri_segments']['tasks'] )) {
					$this->segments = $this->session->userdata['uri_segments']['tasks'];
				...
				$this->session->userdata['uri_segments']['tasks'] = $this->segments;
				$this->session->sess_write();
				$this->logToFile('steve.txt', 'After saving to session after filling default values:' );
				$this->logToFile('steve.txt', print_r($this->session->userdata('tasks_uri_segments'), TRUE) );
			}
		} else {
			$this->segments = fill_default_values($this->segments, $this->default_segments);
			$this->session->userdata['uri_segments']['tasks'] = $this->segments;
    	...
    	$this->action = "list";
		$this->template->assign( 'data', $this->model_tasks->lister( $this->segments, $this->pagination_per_page ) );
		
		
		$this->template->assign( 'pager', $this->pager($this->segments) );
		$this->template->assign( 'partial_path', 'test/tasks' );
    ...
    function show( $id )
    {
		...
		$this->template->assign( 'data', $this->model_tasks->get( $id ) );
    ...
    function delete( $id = FALSE )
    {
        switch ( $_SERVER ['REQUEST_METHOD'] )
        {
            case 'GET':
                ...
                $this->model_tasks->delete( $id );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
 			case 'POST':
 				//delete from submit button on list using checked boxes
                $this->model_tasks->delete( $this->input->post('delete_ids') );
                redirect( $_SERVER['HTTP_REFERER'] );
            break;
		}
	}
    ...
    protected function setRules()
	{
		...
		$this->load->library('form_validation');
						$this->form_validation->set_rules( 'episode_id', lang('episode_id'), 'required|max_length[10]|integer' );
				$this->form_validation->set_rules( 'task', lang('task'), 'required|max_length[50]' );
				$this->form_validation->set_rules( 'create_timestamp', lang('create_timestamp'), 'required' );
	...
	{
						$data_post['episode_id'] = $this->input->post( 'episode_id' );
				$data_post['task'] = $this->input->post( 'task' );
				$data_post['create_timestamp'] = $this->input->post( 'create_timestamp' );
				$data_post['complete_timestamp'] = $this->input->post( 'complete_timestamp' );
	...
	{	
		...
		$this->template->assign( 'fields', $this->model_tasks->fields( $includePrimaryIDfield ) );
	...
	protected function displayFormForEdit($id){
		if ($this->action == "edit")
		{
			$this->model_tasks->raw_data = TRUE;
			$this->template->assign( 'data', $this->model_tasks->get( $id ) );
	...
	protected function saveFormData($id){
		switch ($this->action) {
			case 'create':
				...
				$insert_id = $this->model_tasks->insert( $data );
				
				redirect(base_url() . 'test/tasks/show/' . $insert_id );
				break;
			case 'edit':
				$this->model_tasks->update( $id, $this->getPostData('save') );
				redirect(base_url() . 'test/tasks/show/' . $id );   
				break;
		}	
	...
	protected function commonAssignments()
	{
		...
		$this->template->assign( 'metadata', $this->model_tasks->metadata() );
	}
	...
	protected function pager(){
        ...
        {
        	$this->total_rows = $this->model_tasks->total_rows($this->segments['search_text']);
        	unset($this->segments['page']);
        	
            $config = array();
            $config['uri_segment'] = 9;
			$config['total_rows'] = $this->total_rows;
			$config['base_url'] = base_url() . 'test/tasks/index' .  uri_segments($this->segments) .  '/page/';
